...
class App:
    ...
    def _setup_default_keybindings(self):
        ...
        self.keybindings.bind("r", lambda ctx, e: Term.clear() or True)
        self.keybindings.bind("q", lambda ctx, e: self._quit() or True)

        # Late: default focus navigation (run only if widgets didn't eat the event)
        self.keybindings.bind(Key.UP,   self._nav_prev, phase="late")
        self.keybindings.bind(Key.DOWN, self._nav_next, phase="late")
